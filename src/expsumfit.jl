...
function expsum_init(
    ...
    return ExpSumFitInit(n, m, zeros(T, nY, mY), zeros(T, nY, n - 1), zeros(T, mY),
        diagm(-1 => ones(T, n - 1)), # companion matrix
        zeros(Complex{T}, len, n + withconst),
        zeros(T, len, n + withconst), T.(calc_integral_rules(1:n, m = m)))
end
...
Interpolation order is `m`.
  ...
  - `n=1`, `m=2` [Simpson's 1/3 rule](https://en.wikipedia.org/wiki/Simpson%27s_rule#Simpson%27s_1/3_rule)
  - `n=1`, `m=3` [Simpson's second (3/8) rule](https://en.wikipedia.org/wiki/Simpson%27s_rule#Simpson%27s_3/8_rule)
"""
